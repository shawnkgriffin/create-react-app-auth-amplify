...
function createNewProject(
  ...
  let newProject = require('./projectSchema.js/index.js');
  if (template !== null)
...
function postProject(newProject, callback) {
  axios
    .post(
      `https://us-central1-project-534d9.cloudfunctions.net/api/project`,
      newProject,
    )
    .then(response => {
...
function convertCSVtoJSON() {
  ...
  csv.forEach(row => {
    if (row.StepNumber && row.QuestionNumber === null) {
      // push the step info
      project.pages.push({
        name: row.StepLabel,
        elements: [
    ...
    } else if (row.StepNumber && row.QuestionNumber) {
      // push the question info
...
async function getProjects(user, callback) {
  axios
    .get(
      `https://us-central1-project-534d9.cloudfunctions.net/api/userprojects/${user}`,
    )
    .then(data => {
...
async function getTemplates(user, callback) {
  axios
    .get(
      `https://us-central1-project-534d9.cloudfunctions.net/api/templates`,
    )
    .then(data => {
...
function putProject(project, callback) {
  axios
    .put(
      `https://us-central1-project-534d9.cloudfunctions.net/api/project/${project.id}`,
      project,
    )
    .then(response => {
      callback(response);
...
function deleteProject(id, callback) {
  axios
    .delete(
      `https://us-central1-project-534d9.cloudfunctions.net/api/project/${id}`,
    )
    .then(response => {
      callback(response);
    })
